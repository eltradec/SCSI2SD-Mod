...
void sdCheckPresent()
{
		...
		(sdCmdState == CMD_STATE_IDLE))
	{
		...
		if (1 && !(blockDev.state & DISK_PRESENT))
        // RM changed from cs to 1 in case CS is not bidirectional
		{
		...
		else if (0 && (blockDev.state & DISK_PRESENT))
        // RM changed from !cs to 0; assume disk is always inserted
		{
			sdDev.capacity = 0;
